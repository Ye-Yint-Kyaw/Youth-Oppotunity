...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['profile_img']) && $_FILES['profile_img']['error'] == 0) {
        ...
        $user_folder_img = "uploads/profile/company_" . $company_id;
        ...
        if (move_uploaded_file($_FILES['profile_img']['tmp_name'], $target_file_img)) {
            $stmt_img = $conn->prepare("UPDATE company SET profile = ? WHERE id = ?");
            $stmt_img->bind_param('si', $target_file_img, $company_id);
            if (!$stmt_img->execute()) {
    ...
    $stmt = $conn->prepare("UPDATE company SET company_name = ?, email = ?, phone_number = ? WHERE id = ?");
    $stmt->bind_param('sssi', $name, $email, $phone_number, $company_id);
    if ($stmt->execute()) {
        ...
        <div class="form-body">
            <form id="editForm" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($user['company_name']); ?>">
                </div>
                <div class="form-group">
